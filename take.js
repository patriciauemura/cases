...
var g_client_name = document.querySelector('title').innerText.split(':')[1].trim().split(' ')[0]
...
// Clica no botão para criar email
// document.querySelector('[aria-label="Create new email"]').click()

// Retorna o novo email a ser editado
function myCase() {
    return Array.from(document.querySelectorAll('[id="email-body-content-top-content"]')).pop()
}

function myReplace(input, replace) {
  myCase().innerHTML = myCase().innerHTML.replace(input, replace)
}
